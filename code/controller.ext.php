...
class module_controller {
	...
	static function doTaskServer(){
		...
		switch($formvars['InTask']) {
			case 'kickServer':
				$return = self::taskKickServer($formvars['port'],$currentuser['username']);
				break;
			case 'startServer':
				$return = self::taskStartServer($formvars['port'],$currentuser['username']);
				break;
			case 'stopServer':
				$return = self::taskStopServer($formvars['port'],$currentuser['username']);
				break;
			case 'editServer':
        		header("location: ./?module=" . $controller->GetCurrentModule() . "&show=Edit&port=".$formvars['port']);
		        exit;
				break;				
			default:
	...
	static function doSaveServer(){
		...
		elseif(file_exists("/etc/sentora/panel/modules/shoutcast_admin/servers/".$currentuser['username']."_".$formvars['port'].".conf")){ 
			header("location: ./?module=" . $controller->GetCurrentModule() . "&saved=Server exists");		
		}
		else{
			if(file_exists("/etc/sentora/panel/modules/shoutcast_admin/srv/".$currentuser['username']."_".$formvars['port'].".conf")){
				self::deletefile("/etc/sentora/panel/modules/shoutcast_admin/srv/".$currentuser['username']."_".$formvars['port'].".conf");
				self::deletefile("/etc/sentora/panel/modules/shoutcast_admin/srv/".$currentuser['username']."_".$formvars['port'].".php");		
			}
			$fp = fopen("/etc/sentora/panel/modules/shoutcast_admin/srv/".$currentuser['username']."_".$formvars['port'].".conf", 'w+');
$input=";
...
BanFile=/etc/sentora/panel/modules/shoutcast_admin/srv/".$currentuser['username']."_".$formvars['port'].".ban
LogFile=/etc/sentora/panel/modules/shoutcast_admin/srv/".$currentuser['username']."_".$formvars['port'].".log
W3CLog=/etc/sentora/panel/modules/shoutcast_admin/srv/".$currentuser['username']."_".$formvars['port'].".w3c.log
MetaInterval=32768";		
			fwrite($fp, $input);
			fclose($fp);			
			$fp = fopen("/etc/sentora/panel/modules/shoutcast_admin/srv/".$currentuser['username']."_".$formvars['port'].".php", 'w+');
$input="<?php
